...
class Mailer @Inject() (configuration:Configuration, mailer:MailerClient) {
  ...
  def welcome(profile:Profile, link:String)(implicit messages:Messages) = {
    sendEmailAsync(profile.email.get)(
      ...
      bodyHtml = Some(be.frol.playrestauthenticator.views.html.mails.welcome(profile.firstName.get, link).toString),
      bodyText = Some(be.frol.playrestauthenticator.views.html.mails.welcomeText(profile.firstName.get, link).toString)
    )
  }
  ...
  def resetPassword(email:String, link:String)(implicit messages:Messages) = {
    sendEmailAsync(email)(
      ...
      bodyHtml = Some(be.frol.playrestauthenticator.views.html.mails.resetPassword(email, link).toString),
      bodyText = Some(be.frol.playrestauthenticator.views.html.mails.resetPasswordText(email, link).toString)
    )
  }
}