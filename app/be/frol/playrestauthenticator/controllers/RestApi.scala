...
import be.frol.playrestauthenticator.errors.{AuthenticationException, UnauthorizedException}
...
class RestApi @Inject() (
  ...
  def profile = SecuredAction.async { implicit request =>
    ...
    prunedJson.fold(
      _ => Future.successful(InternalServerError(Json.obj("be/frol/playrestauthenticator/errors" -> Messages("error.profileError")))),
